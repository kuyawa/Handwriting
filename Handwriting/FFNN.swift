...
public final class FFNN {
    ...
    var errorFunction: ErrorFunction = .Default(average: false)
...
public extension FFNN {
    ...
    /// Returns an URL for a document with the given filename in the default documents directory.
    ...
    public static func read(url: URL) -> FFNN? {
...
func randomWeight(numInputNodes: Int) -> Float {
...
func crossEntropy(a: Float, b: Float) -> Float {
    return log(a) * b
}
...
func linearDerivative(_ y: Float) -> Float {
    return 1.0
}
...
func sigmoidDerivative(_ y: Float) -> Float {
    return y * (1 - y)
}

/// Rational sigmoid activation function.
func rationalSigmoid(_ x: Float) -> Float {
    return x / (1.0 + sqrt(1.0 + x * x))
}

/// Derivative for the rational sigmoid activation function.
func rationalSigmoidDerivative(_ y: Float) -> Float {
    let x = -(2 * y) / (y * y - 1)
    return 1 / ((x * x) + sqrt((x * x) + 1) + 1)
}

/// Hyperbolic tangent activation function.
func hyperbolicTangent(_ x: Float) -> Float {
    return tanh(x)
}

/// Derivative for the hyperbolic tangent activation function.
func hyperbolicTangentDerivative(_ y: Float) -> Float {
    return 1 - (y * y)
}
...
/*
 NOTE: This extends `FFNN` to adopt the `Storage` protocol, for reading/writing neural networks to file.
 */
...
public protocol Storage {
    ...
    static func fromFile(url: URL) -> StorageType?
...
extension FFNN: Storage {
    ...
    /// Reads a FFNN from file.
    /// - Parameter url: The `URL` for the file to read.
    ...
    public func writeToFile(url: URL) {
        self.write(url: url)
    }
